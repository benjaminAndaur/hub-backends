...
from src.models.mantencion import MantencionCreate, MantencionResponse, MantencionUpdate
...
class MantencionService:
    ...
    async def get_all_mantenciones(
    ...
    async def update_mantencion(
        self, id: int, data: MantencionUpdate
    ) -> Optional[MantencionResponse]:
        db_obj = await self.repository.get_by_id(id)
        if not db_obj:
            return None
