...
class OrdenTrabajoDB(Base):
    ...
    mantencion_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("mantenciones.id", ondelete="CASCADE"), nullable=False
    )
...
class OrdenTrabajoRepuestoDB(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    ot_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("ordenes_trabajo.id", ondelete="CASCADE"), nullable=False
    )
    producto_id: Mapped[int] = mapped_column(
        BigInteger, nullable=False
    )  # Ref a Bodega (productos.id)
    ...
    estado_devolucion: Mapped[str] = mapped_column(
        String(50), default="Ninguna", nullable=False
    )  # Ninguna, Pendiente, Verificada
