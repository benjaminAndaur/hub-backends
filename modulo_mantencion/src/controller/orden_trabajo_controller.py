...
def create_orden_trabajo_blueprint():
    ...
    async def agregar_repuesto(ot_id):
        data = await request.get_json()
        try:
            nuevo = await g.service_ot.agregar_repuesto_ot(
                ot_id, data["producto_id"], data["cantidad"]
            )
    ...
    async def devolver_repuesto(repuesto_id):
        data = await request.get_json()
        try:
            res = await g.service_ot.solicitar_devolucion_repuesto(
                repuesto_id, data["cantidad_devuelta"]
            )
